...
PeerServer.prototype._initializeWSS = function() {
  ...
  this._wss.on('connection', function(socket) {
    ...
    var key = query.key;
    var forwarded_ip = socket.upgradeReq.headers['x-forwarded-for']

    // for nginx environment
    var ip = forwarded_ip || socket.upgradeReq.socket.remoteAddress;
...
PeerServer.prototype._configureWS = function(socket, key, id, token) {
  ...
  var pingTimer; // interval timer for ws ping
  var flagPong = true;
  ...
  if (token === client.token) {
    ...
    if (!this._options.disable_xhr && client.res) {
      client.res.end();
    }
  } else {
  ...
  // set interval timer for ping.
  pingTimer = setInterval(function(ev){
    if(flagPong) {
      socket.ping();
      util.log("sent ping for %s", id);
      flagPong = false;
    } else {
      if(pingTimer){
        clearInterval(pingTimer);
  ...
  }, this._options.ping_timer);

  // event listener for pong
  socket.on('pong', function() {
    flagPong = true;
    util.log('receive pong from %s', id);
  });

  // We're going to emit here, because for XHR we don't *know* when someone
  // disconnects.
  this.emit('connection', {id: id, client_ip: client.ip});
};
...
PeerServer.prototype._initializeHTTP = function() {
  ...
  this._app.post(this._options.path + ':key/:id/:token/id', function(req, res, next) {
    if(!self._options.disable_xhr) {
      ...
      // when xhr streaming is disabled
      res.send(404);
      return next();
    }
  });
...
PeerServer.prototype._removePeer = function(key, id) {
  if (this._clients[key] && this._clients[key][id]) {
    this.emit('disconnect', {id: id, client_ip: this._clients[key][id].ip});
...
PeerServer.prototype._handleTransmission = function(key, message) {
  ...
  if (destination) {
    try {
      ...
      } else if (!this._options.disable_xhr && destination.res) {
        data += '\n';
